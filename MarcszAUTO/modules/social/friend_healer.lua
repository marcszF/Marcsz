-- =========================================
-- MODULE: Friend Healer (SIO System)
-- =========================================
-- Heal party members with spells or items
...
local healItem
macro(200, function()
    if storage[panelName].enabled and storage[panelName].spellName:len() > 0 
       and manapercent() > storage[panelName].minMana then
        
        for _, spec in ipairs(getSpectators()) do
            if not spec:isLocalPlayer() then
                if spec:isPlayer() and storage[panelName].minFriendHp >= spec:getHealthPercent() 
                   and isFriend(spec:getName()) then
                    ...
                    if storage[panelName].itemHeal 
                       and distanceFromPlayer(spec:getPosition()) <= storage[panelName].distance 
                       and healItem then
